...
* EJB는 관심사를 적절히 분리하지 못하는 아키텍처의 한 예이다.                 
* EJB 아키텍처는 관심사를 적절히 분리하지 못했기에 유기적인 성장이 어려웠다.           
    
> **잠깐!!** 코드와 밑에 있는 설명은 중요한 부분이 아닙니다.       
> 코드와 밑에 있는 설명은 얼마나 강하게 결합되었는지를 말해주는 상황입니다.         
> 그러니 이 부분에 대한 자세한 설명은 넘어가고 요점만 정리해보겠습니다. 
...
[이 부분 내용 참고](https://github.com/Yooii-Studios/Clean-Code/blob/master/Chapter%2011%20-%20%EC%8B%9C%EC%8A%A4%ED%85%9C.md#3-1)
 
## 횡단 관심사 
> 관심사를 분리하지 못해 객체 지향의 뿌리를 흔들었던 EJB 아키텍처          
> 하지만, EJB2 아키텍처는 일부 영역에서 관심사를 거의 완벽하게 분리했다.                
> 원하는 트랜잭션, 보안, 일부 영속적인 동작은 소스 코드가 아니라 배치 기술자에서 정의했다.        
         
[영속성](https://hyeooona825.tistory.com/87)과 같은 관심사는 애플리케이션의 자연스러운 객체 경계를 넘나드는 경향이 있다.      
