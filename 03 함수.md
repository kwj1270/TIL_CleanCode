...
* 메서드의 이름은 동 동적인 느낌이 들도록 서술적인 이름을 지어주는 것이 좋다. 
  * `testableHtml()` -> `setupTreadownIncluder()`
  * private 함수라도 서술적인 이름을 지어주는 것이 좋다.
* 함수를 작게 만들라는 이유도 **함수가 작고 단순할 수록 서술적인 이름을 고르기도 쉽기 때문이다.**    
  * 이름은 길어도 상관없다. (오히려 자세히 설명하는 이름을 지정하면 식별하는데 더 도움이 된다.)   
