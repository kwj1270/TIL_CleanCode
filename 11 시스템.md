...
## 관심사를 분리하자 
시작 단계는 **모든 애플리케이션 관심사를 분리하는 것**이다.   
`관심사 분리`는 우리 분야에서 가장 오래되고 가장 중요한 설계 기법 중 하나이다.   
...
```
* 위 코드는 초기화 지연 혹은 계산 지연이라는 기법이다.
* 장점 : 
  * 초기화 지연 혹은 계산 지연이라는 기법이다.
  * 어떤 경우라도 null 포인터를 반환하지 않는다.   
