...
```    
* 위 코드는 `초기화 지연` 혹은 `계산 지연`이라는 기법이다.                   
* `nullPointer`를 반환하지 않고 한 개의 객체만 사용한다는 장점이 있다.                
    
**하지만, `getService()` 메스드는 의존적인 메서드이다.**    
...
- 왜냐하면 지연 생성을 하더라도 컴파일은 되지 않나 생각하기 때문입니다.    
```    

테스트도 문제다.   
MyServiceImpl이 무거운 객체라면 단위 테스트에서 
Service 메서드를 호출하기 이전에 적절한 테스트 전용 객체를 service 필드에 할당해야한다.    
