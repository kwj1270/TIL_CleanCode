...
# 의사결정을 최적화하라    
충분히 큰 시스템에서는(그것이 도시이건 소프트웨어이건) 한 사람이 모든 결정을 내릴 수는 없다.  시기가 되었을 경우 해당 파트의 책임자(여기서는 를 뜻한다)에게 맡기는 것이 불필요한 고객 피드백과 고통을 덜어줄 것이다.
...
성급한 결정은 불충분한 지식으로 내린 결정인 것을 명시하자.                    
