...
그러므로 우리는 `깨끗한 클래스`에 대해서 공부해야 한다.         
...
그렇다면 **"얼마나 작게 만들어야 하는가?"**        
답은 **클래스는 클래스가 맡은 책임을 센다.**
...
* 그만큼 함수를 쪼개기 쉬워진다.   
* 하지만 아쉽게도 이같은 방법을 취하면 응집력을 잃는다.   
* 모든 메서드가 아닌 특정 메서드만이 특정 인스턴스 변수를 사용하기 때문이다.   
    
**잠깐!**     
몇몇 함수가 몇몇 변수만 사용한다면 독자적인 클래스로 분리해도 되지 않을까?             
당연하다. **클래스가 응집력을 잃는다면 독자적인 클래스로 쪼개자!**                 
...
아래 코드의 큰 함수를 작은 함수/클래스 여럿으로 쪼개보도록 연습해보도록 하자.
...
**재구현이 아니다!**        
프로그램을 다시 짜지 않았다.       
실제 리펙토링 전/후 프로그램을 살펴보면 알고리즘과 동작 원리가 동일하다.      
...
메서드를 훍어보면 비공개 메서드가 있는데 select 문을 처리할 때만 사용된다.        
저자의 경험상 클래스 일부에서만 사용되는 비공개 메서드는 개선할 여지를 시사한다.        
그렇기에 클래스에 손대는 순간 설계를 개선하려는 고민과 시도가 필요하다.     
(논리적으로 완성되거나 추가할 일이 없다면 내버려두는 것도 낫다)         
...
요구사항은 변하기 마련이다.   
따라서 코드도 변하기 마련이다.   
객체 지향 프로그래밍 입문에서 우리는 구체적인concreate 클래스와 추상abstract클래스가 있다고 배웠다.     
구체적인 클래스는 상세한 구현을 포함하며   
추상 클래스는 개념만 포함한다고 배웠다.    
