...
또한, 단항 함수 형식은 입력 인수로 시스템을 변경하기 때문에 이벤트 함수에서 유용하게 사용됩니다.     
...
이항 함수가 나쁘다는 것이 아니라 단항 함수에 비해 위험 요소가 증가하니 이를 최대한 개선시키면서 개발하고자 해야 합니다.     
...
* 이항, 삼항 등에 사용하는 인수가 동등할 경우 가변 인수로 표현하는 것이 더 좋을 수 있다.  
* 하지만 그 이상으로 넘어갈 경우 애초에 로직이 복잡해지므로 이럴때는 사용하지 않는 것이 좋다.      
